...
async function adminAddBalanceController(req, res) {
  ...
  return res.status(200).json({
    ...
    perModelSpecTokenCredits: record.perModelSpecTokenCredits,
  });
}
  ...
  if (spec) {
    ...
    if (modelSpec?.balance?.enabled) {
      record = await Balance.findOneAndUpdate(
        { user: user._id },
        { $set: { [`perModelSpecTokenCredits.${kebabCase(spec)}`]: amount } },
...
async function getBalanceController(req, res) {
  const balanceData = await Balance.findOne(
    { user: req.user.id },
    '-_id tokenCredits perModelSpecTokenCredits autoRefillEnabled refillIntervalValue refillIntervalUnit lastRefill refillAmount',
  ).lean();
